...
namespace Solution.Infrastructure.Tests
{
    ...
    public class DatabaseUnitOfWorkTest
    {
        ...
        {
            Assert.IsNotNull(DatabaseUnitOfWork.UserRepository.FirstOrDefault<AuthenticatedModel>(w => w.UserId == 1L));
        }

        [TestMethod]
        public void DatabaseUnitOfWorkUserRepositoryFirstOrDefaultResultAsynchronous()
        {
            Assert.IsNotNull(DatabaseUnitOfWork.UserRepository.FirstOrDefaultAsync<AuthenticatedModel>(w => w.UserId == 1L));
        }
        ...
        {
            Assert.IsNotNull(DatabaseUnitOfWork.UserRepository.SingleOrDefault<AuthenticatedModel>(w => w.UserId == 1L));
        }

        [TestMethod]
        public void DatabaseUnitOfWorkUserRepositorySingleOrDefaultResultAsynchronous()
        {
            Assert.IsNotNull(DatabaseUnitOfWork.UserRepository.SingleOrDefaultAsync<AuthenticatedModel>(w => w.UserId == 1L));
        }
